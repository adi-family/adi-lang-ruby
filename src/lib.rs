...
extern "C" fn plugin_info() -> PluginInfo {
    PluginInfo::new("adi.lang.ruby", "Ruby Language Support", env!("CARGO_PKG_VERSION"), "language")
...
extern "C" fn plugin_init(ctx: *mut PluginContext) -> i32 {
    unsafe {
        let host = (*ctx).host();
        let descriptor = ServiceDescriptor::new(SERVICE_ID, ServiceVersion::new(1, 0, 0), "adi.lang.ruby")
        ...
        if let Err(code) = host.register_svc(descriptor, handle) {
            host.error(&format!("Failed to register Ruby analyzer service: {}", code));
            return code;
        }
